...
class AnswerTest(BaseTestMixins, TestCase):
    ...
    def test_user_must_login_to_accept_answer(self):
        response = self.client.post(reverse('answers:toggle_accept'),
                                    data={'answer_id': 1})
        response_message = response.json()['response']
        self.assertEqual(response.status_code, 400)
        self.assertIn('Please login or signup before doing this.',
    ...
    def test_cannot_accept_multiple_answers(self):
        ...
        second_user = get_user_model().objects.get(username=username)
        Answer.objects.create(user=second_user,
                              question=question,
                              content='test second answer')
        ...
        self.client.post(reverse('answers:toggle_accept'),
                         data={'answer_id': first_answer.id})
        first_answer.refresh_from_db()
        self.assertEqual(first_answer.accepted, True)

        # accept the second answer and confirm that only one answer
        # in total is accepted
        second_answer = question.answer_set.last()
        self.client.post(reverse('answers:toggle_accept'),
                         data={'answer_id': second_answer.id})
        second_answer.refresh_from_db()
        self.assertEqual(second_answer.accepted, True)
        num_accepted_answers = Answer.objects.filter(question=question,
                                                     accepted=True).count()
